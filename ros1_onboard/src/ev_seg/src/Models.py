...
class U_Net_hybrid(nn.Module):
    """
    UNet - Basic Implementation
    Paper : https://arxiv.org/abs/1505.04597
    """
    def __init__(self, in_ch=3, out_ch=1):
        super(U_Net_hybrid, self).__init__()
        ...
        self.Conv3 = conv_block(filters[1], filters[2])
        self.Conv4 = conv_block(filters[2], filters[3])
        self.Conv5 = conv_block(filters[3]*2, filters[4])

        self.Up5 = up_conv(filters[4], filters[3])
        self.Up_conv5 = conv_block(filters[3]*3, filters[3])
    ...
    def forward(self, x):
        ...
        rgb = x[:,0:3,:,:]
        event = x[:,3:6,:,:]
        e1_rgb = self.Conv1(rgb)

        e2_rgb = self.Maxpool1(e1_rgb)
        e2_rgb = self.Conv2(e2_rgb)

        e3_rgb = self.Maxpool2(e2_rgb)
        e3_rgb = self.Conv3(e3_rgb)

        e4_rgb = self.Maxpool3(e3_rgb)
        e4_rgb = self.Conv4(e4_rgb)
        
        e1_event = self.Conv1(event)

        e2_event = self.Maxpool1(e1_event)
        e2_event = self.Conv2(e2_event)

        e3_event = self.Maxpool2(e2_event)
        e3_event = self.Conv3(e3_event)

        e4_event = self.Maxpool3(e3_event)
        e4_event = self.Conv4(e4_event)

        e4 = torch.cat((e4_rgb, e4_event), dim=1)
        e5 = self.Maxpool4(e4)
        e5 = self.Conv5(e5)
        ...
        d4 = self.Up4(d5)
        d4 = torch.cat((e3_rgb, e3_event,d4), dim=1)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        d3 = torch.cat((e2_rgb,e2_event, d3), dim=1)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        d2 = torch.cat((e1_rgb,e1_event, d2), dim=1)
        d2 = self.Up_conv2(d2)
